...
export default {
    ...
    computed:{
        ...
        ...mapGetters('textileManagement',['photo']),
        filterTextile(){
            return this.textiles.filter((item) => {
    ...
    methods:{
        ...mapActions('textileManagement',['getPhotoAsync','deletePhotoAsync','uploadPhotoAsync']),
        getPhoto(){
    ...
    <div class="mt-5">
        <v-row>
            <v-select
                v-model="selectedId"
                :items="filterTextile"
                :label="$t(`${trans_prefix}.textile.select`)"
                item-value="id"
                item-text="name"
                @change="getPhoto"
            >
